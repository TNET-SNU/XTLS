...
#define MAX_TLS_CRYPTO_OVERHEAD 2048
#define TLS_EXTRA_NONCE_LEN sizeof(uint64_t)
#define MAX_RECORD_SIZE (RECORD_HEADER_SIZE + MAX_TLS_CRYPTO_OVERHEAD + MAX_PLAIN_DATA_LEN + TLS_EXTRA_NONCE_LEN)
#define MAX_HANDSHAKE_LENGTH 4096
#define MAX_KEY_SIZE 128
#define MAX_KEY_BLOCK_LEN (256 * 6)
#define MAX_IV_SIZE 16
#define MAX_CERT_SIZE 32 /* assume P256 cert */
#define MAX_CERT_SIG_SIZE 128 /* assume P256 cert */
#define MAC_SIZE 20
#define MAX_OUTSTANDING_PKA_REQ 64
...
static const cipher_suite_t TLS_AES_256_GCM_SHA384 = {{0x13, 0x02}}; /* for TLS 1.3 */
...
struct ec_point_formats {
    uint16_t extension_type;
    uint16_t extension_length;
    uint8_t ec_point_format_list_length;
    uint8_t *ec_point_format_list;
} ec_point_formats_t;
...
struct supported_groups {
    uint16_t extension_type;
    uint16_t extension_length;
    uint16_t curves_list_length;
    uint16_t *curves_list;
} supported_groups_t;
...
static const supported_group_t X25519 = {{0x00, 0x1d}};
static const supported_group_t SUPPORTED_GROUP_UNKNOWN = {{0x00, 0x00}};

#define X25519_KEY_LEN 32
#define curve_type_X25519 0x001d

#define COMPARE_SUPPORTED_GROUP(x, y) ( \
    ((x.sg[0] == y.sg[0]) && (x.sg[1] == y.sg[1])) ? 1 : 0 \
)
...
typedef
struct encrypt_then_mac {
    uint16_t extension_type;
    uint16_t extension_length;
} encrypt_then_mac_t;

typedef
struct extended_master_secret {
    uint16_t extension_type;
    uint16_t extension_length;
} extended_master_secret_t;
...
struct signature_algorithms {
    ...
    uint16_t *signature_algorithm_list;
...
static const signature_algorithm_t ECDSA_SECP256r1_SHA256 = {{0x04, 0x03}};
static const signature_algorithm_t SIGNATURE_UNKNOWN      = {{0x00, 0x00}};

#define COMPARE_SIG_ALGO(x, y) ( \
    ((x.sa[0] == y.sa[0]) && (x.sa[1] == y.sa[1])) ? 1 : 0 \
)

typedef
struct client_supported_versions {
    uint16_t extension_type;
    uint16_t extension_length;
    uint8_t versions_length;
    protocol_version_t *versions;
} client_supported_versions_t;

typedef
struct psk_key_exchange_modes {
    uint16_t extension_type;
    uint16_t extension_length;
    uint8_t psk_key_exchange_mode_list_length;
    uint8_t *psk_key_exchange_mode_list;
} psk_key_exchange_modes_t;

typedef
struct client_key_share {
    uint16_t extension_type;
    uint16_t extension_length;
...
typedef
struct client_pub_key {
    uint8_t cli_pub_key[X25519_KEY_LEN]; /* assume X25519 */
} client_pub_key_t;

typedef
struct client_extensions {
    server_name_t server_name;
    ec_point_formats_t ec_point_formats;
    supported_groups_t supported_groups; /* essential */
    session_ticket_t session_ticket;
    encrypt_then_mac_t encrypt_then_mac;
    extended_master_secret_t extended_master_secret;
    signature_algorithms_t signature_algorithms; /* essential */
    client_supported_versions_t supported_versions; /* essential */
    psk_key_exchange_modes_t psk_key_exchange_modes;
    client_key_share_t key_share; /* essential */
} client_extensions_t;
...
typedef
struct server_supported_versions {
    uint16_t extension_type;
    uint16_t extension_length;
    protocol_version_t version;
} server_supported_versions_t;

typedef
struct server_key_share {
    uint16_t extension_type;
    uint16_t extension_length;
    uint16_t curve_type;
    uint16_t pub_key_length;
    uint8_t server_pub_key[X25519_KEY_LEN];
} server_key_share_t;

typedef
struct server_extensions {
    server_supported_versions_t supported_versions; /* essential */
    server_key_share_t key_share; /* essential */
} server_extensions_t;
...
struct server_hello {
    ...
    uint16_t extensions_length;
    server_extensions_t extensions;
} server_hello_t;

typedef
struct server_encrypted_extensions {
    uint16_t extensions_length;
    server_extensions_t extensions;
} server_encrypted_extensions_t;
...
struct security_params {
    ...
    supported_group_t           curve;
    signature_algorithm_t       signature_algorithm;
...
struct certificate_list {
    uint24_t certificates_length;
    certificate_t *certificates;
} certificate_list_t;

typedef
struct certificate_list_tls13 {
    uint8_t request_context_length;
    uint24_t certificates_length;
    certificate_t *certificates;
    uint16_t cert_extensions_length;
} certificate_list_tls13_t;

typedef
struct certificate_verify_tls13 {
    signature_algorithm_t signature_algorithm;
...
typedef
struct premaster_secret {
    protocol_version_t version;
    uint8_t random[46];
} premaster_secret_t;

typedef
struct encrypted_premaster_secret {
    uint8_t *encrypted_premaster_secret;
} encrypted_premaster_secret_t;

typedef
struct client_diffie_hellman_public {
    uint8_t *data;
...
struct handshake {
    ...
    union {
        ...
        server_encrypted_extensions_t server_encrypted_extensions;
        certificate_list_t certificate;
        certificate_list_tls13_t certificate_tls13;
        cert_verify_tls13_t certificate_verify;
        client_key_exchange_t client_key_exchange;
        finished_t client_finished;
        finished_t server_finished;
        finished_tls13_t server_finished_tls13;
        finished_tls13_t client_finished_tls13;
    } body;
} handshake_t;
...
typedef
struct tls13_ctx {
    int     handshake_key_calculated;
    int     cert_signatured;
    uint8_t handshake_secret[MAX_KEY_SIZE];
    uint8_t client_secret[MAX_KEY_SIZE];
    uint8_t server_secret[MAX_KEY_SIZE];
    uint8_t client_handshake_key[MAX_KEY_SIZE];
    uint8_t server_handshake_key[MAX_KEY_SIZE];
    uint8_t client_handshake_iv[MAX_KEY_SIZE];
    uint8_t server_handshake_iv[MAX_KEY_SIZE];
    uint8_t client_application_key[MAX_KEY_SIZE];
    uint8_t server_application_key[MAX_KEY_SIZE];
    uint8_t client_application_iv[MAX_KEY_SIZE];
    uint8_t server_application_iv[MAX_KEY_SIZE];
    uint8_t certificate_hash[256]; /* for debugging */
    uint8_t certificate_hash_len; /* for debugging */
    uint8_t cert_signature[MAX_CERT_SIG_SIZE];
    uint8_t cert_signature_len;
    uint8_t calculated_vd[48];
} tls13_ctx_t;
