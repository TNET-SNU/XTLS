...
#define SVR_CCS_FIN_SEG_LEN                                                    \
    51 /* 51: TCP seg len of server change cipher spec, finished*/
#define CLI_KEX_CCS_FIN_SEG_LEN                                                \
    318                      /* Need to remove (by dynamic HTTPS req len) */
#define SVR_RECS_SEG_LEN 848 /* Need to remove (by dynamic signature len) */
#define CLI_CCCS_CF_SEG_LEN                                                    \
    291 /* Need to remove (by dynamic HTTPS req len)                           \
         */
...
#define HEALTH_CHECK        10000 /* unit: us */
#define RM_SOCK_INTERVAL    10000 /* unit: us */
...
typedef struct tcp_connection {
    ...
    struct timespec hws_start;
    struct timespec hws_end;
    ...
    uint32_t next_sent_seq; // little endian

    int need_to_send_rst_meta;
...
typedef struct meta_hdr meta_hdr_t;

typedef union ssl_crypto_opcode ssl_crypto_opcode_t;
...
typedef struct ssl_session {
    ...
    pka_operand_t *shared_secret_operand;
    pka_operand_t *ecdsa_transcript_hash;
    pka_operand_t *ecdsa_k;
    pka_operand_t *ecdhe_priv_key;
    pka_operand_t *ecdhe_pub_key;

    security_params_t pending_sp;
    security_params_t read_sp;
    security_params_t write_sp;

    uint8_t server_records[1024];
    int server_records_len;

    TAILQ_HEAD(recv_q_head, record) recv_q;
    int recv_q_cnt;

    void *pending_pka_op;   /* ssl_crypto_op_t* */
    void *pending_pka_op_1; /* ssl_crypto_op_t* */
    int num_cke_retransmitted;

    int num_of_retransmitted_ch;
    int num_of_retransmitted_meta_pkt;

#if VERBOSE_CONN_LAT
    struct timespec key_gen_time;
    struct timespec gen_key_time;
    struct timespec ss_cal_time;
    struct timespec cal_ss_time;
    struct timespec ee_c_cv_gen_time;
    struct timespec gen_cv_time;
    struct timespec app_key_cal_time;
    struct timespec cal_app_key_time;
...
typedef struct dpdk_private_context {
...
extern uint64_t ring_eq_min;
extern uint64_t ring_eq_max;
extern uint64_t ring_eq_sum;
extern uint64_t ring_eq_cnt;
extern uint64_t ring_eq_fail_cnt;
extern uint64_t ring_eq_success_cnt;

extern uint64_t ring_dq_min;
extern uint64_t ring_dq_max;
extern uint64_t ring_dq_sum;
extern uint64_t ring_dq_cnt;
extern uint64_t ring_dq_fail_cnt;
extern uint64_t ring_dq_success_cnt;
...
int
send_tcp_packet(tcp_connection_t *conn, uint8_t *payload, uint16_t len,
                uint8_t flags, uint8_t offl_flags);
...
uint8_t *
get_wptr(uint16_t core_id, uint16_t port, uint16_t pktsize, int hw_csum,
         int l4_len);
...
create_pattern_template_jump_recv(uint16_t port_id,
                                  struct rte_flow_error *error);

static struct rte_flow_pattern_template *
create_pattern_template_hws_recv(uint16_t port_id,
                                 struct rte_flow_error *error);

static struct rte_flow_actions_template *
create_actions_template_jump_recv(uint16_t port_id,
                                  struct rte_flow_error *error);

static struct rte_flow_actions_template *
create_actions_template_hws_recv(uint16_t port_id,
                                 struct rte_flow_error *error);
...
/* init stage (send) */
static struct rte_flow_pattern_template *
create_pattern_template_hws_send(uint16_t port_id,
                                 struct rte_flow_error *error);

static struct rte_flow_actions_template *
create_actions_template_hws_send(uint16_t port_id,
                                 struct rte_flow_error *error);
...
int
process_ssl_packet(tcp_connection_t *tcp_conn, uint8_t *payload,
                   uint16_t payload_len);
void
