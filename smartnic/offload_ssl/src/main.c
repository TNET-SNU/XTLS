    ...
    .rx_adv_conf =
        {
            .rss_conf = {.rss_key = NULL,
                         .rss_hf = RTE_ETH_RSS_TCP | RTE_ETH_RSS_IP |
...
uint64_t ring_eq_min = UINT64_MAX;
uint64_t ring_eq_max;
uint64_t ring_eq_sum;
uint64_t ring_eq_cnt;
uint64_t ring_eq_fail_cnt;
uint64_t ring_eq_success_cnt;

uint64_t ring_dq_min = UINT64_MAX;
uint64_t ring_dq_max;
uint64_t ring_dq_sum;
uint64_t ring_dq_cnt;
uint64_t ring_dq_fail_cnt;
uint64_t ring_dq_success_cnt;

ecc_info_t *ecc_info[MAX_CPUS];
...
global_init(void)
{
    ...
    // snprintf(original_key_file, sizeof(original_key_file),
    // "%s/cert/rsa_cert_2048.pem", cwd);

    snprintf(original_key_file, sizeof(original_key_file),
             "%s/cert/ecdsa_cert_256.pem", cwd);

    const char *original_key_passwd = "1234";

    static uint8_t key[] = {0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05,
    ...
    for (rxlcore_id = 1; rxlcore_id < num_core; rxlcore_id++) {
        ...
        pktmbuf_pool[rxlcore_id] = rte_pktmbuf_pool_create(
    ...
    RTE_ETH_FOREACH_DEV(portid)
    {
        ...
        ret = rte_eth_dev_configure(portid, num_core - 1, num_core - 1,
                                    &port_conf);
        ...
        for (rxlcore_id = 0; rxlcore_id < num_core - 1; rxlcore_id++) {
            ret = rte_eth_rx_queue_setup(
                portid, rxlcore_id, nb_rxd, rte_eth_dev_socket_id(portid),
        ...
        for (txlcore_id = 0; txlcore_id < num_core - 1; txlcore_id++) {
            ret =
                rte_eth_tx_queue_setup(portid, txlcore_id, nb_txd,
    ...
    sha384 = EVP_MD_fetch(NULL, "SHA384", NULL);

    /* PKA Initialization */
    instance = pka_init_global(
        "ssl offload", PKA_F_PROCESS_MODE_SINGLE | PKA_F_SYNC_MODE_DISABLE,
        PKA_RING_CNT, PKA_QUEUE_CNT, CMD_QUEUE_SIZE, RSLT_QUEUE_SIZE);
    ...
    strncpy(option.key_file, original_key_file, strlen(original_key_file));
    strncpy(option.key_passwd, original_key_passwd,
            strlen(original_key_passwd));

    // if (unlikely(cert_load_key_rsa(&ctx_example) < 0)) {
    //     fprintf(stderr, "cert_load_key_rsa fail\n");
    //     exit(EXIT_FAILURE);
    // }

    if (unlikely(cert_load_key_ecdsa(&ctx_example) < 0)) {
        fprintf(stderr, "cert_load_key_ecdsa fail\n");
        exit(EXIT_FAILURE);
    }
    ...
    RTE_ETH_FOREACH_DEV(portid)
    {
        if (unlikely(rte_flow_flush(portid, &flow_err) < 0)) {
            fprintf(stderr, "Failed to flush flow rules on port %d: %s\n",
                    portid,
                    flow_err.message ? flow_err.message : "(no message)");
        } else {
    ...
    while (-1 != (o = getopt(argc, argv, "m:t:h:"))) {
        ...
        case 't':
            ...
            if (host_threads_num <= 0) {
                ...
                fprintf(stderr, "host_threads_num cannot exceed %d.\n",
                        MAX_HOST_THREADS);
                return false;
            }
            break;
        case 'h':
            fprintf(
                stderr,
                "Usage: %s [EAL options] -- -m max_conn -t host_threads_num\n"
...
main(int argc, char *argv[])
{
    ...
    local_max_conn = max_conn / (rte_lcore_count() - 1);

    local_ring_cnt = 1;

    while (local_ring_cnt <= local_max_conn)
        local_ring_cnt *= 2;
