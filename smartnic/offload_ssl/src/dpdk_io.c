...
send_pkts(uint16_t core_id, uint16_t port) 
{
    ...
    if (dpc->wmbufs[port].len > 0) {
        ...
        if (ret < cnt) {
            ...
            fprintf(stderr, "[CPU %d] [Port %d] tx burst failed, "
...
create_pattern_template_hws_recv(uint16_t port_id, struct rte_flow_error* error)
{
    ...
    // tcp_mask.hdr.tcp_flags = RTE_TCP_PSH_FLAG;
    titems[2].mask = &tcp_mask;
...
create_table_jump_recv(uint16_t port_id) 
{
	...
	if (unlikely(flow_configure(port_id) < 0))
        return NULL;
    fprintf(stderr, "flow_configure() done!\n");
...
rte_flow_pull_and_push(thread_context_t* ctx) 
{
    ...
    for (int i = 0; i < ret; i++) {
        if (result[i].status == RTE_FLOW_OP_SUCCESS) {
            ((tcp_connection_t *)(result[i].user_data))->hws_applied = TRUE;
#if VERBOSE_HWS            
            fprintf(stderr, "hws applied\n");
            fprintf(stderr, "user_data addr = %p\n", result[i].user_data);
            clock_gettime(CLOCK_MONOTONIC, &((tcp_connection_t *)(result[i].user_data))->hws_end);
            fprintf(stderr, "hws lat: %lf\n", (((tcp_connection_t *)(result[i].user_data))->hws_end.tv_nsec - ((tcp_connection_t *)(result[i].user_data))->hws_start.tv_nsec)/1e3);
#endif /* VERBOSE_HWS */
        }
            
        else 
            fprintf(stderr, "flow create failed for client ip: %u, port: %u\n",
    ...
    for (int i = 0; i < ret; i++) {
        if (result[i].status == RTE_FLOW_OP_SUCCESS)
            ((tcp_connection_t *)(result[i].user_data))->hws_deleted = TRUE;
        // else
        //     fprintf(stderr, "flow delete failed for client ip: %u, port: %u\n",
        //             ((tcp_connection_t *)(result[i].user_data))->client_ip,
        //             ((tcp_connection_t *)(result[i].user_data))->client_port);
    }
