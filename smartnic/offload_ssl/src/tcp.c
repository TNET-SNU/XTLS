...
#if VERBOSE_CONN_LAT
int num_ch[MAX_CPUS] = {0};
int num_sh_sc_shd[MAX_CPUS] = {0};
int num_cke[MAX_CPUS] = {0};
int num_rsa_decrypt[MAX_CPUS] = {0};
int num_decrypt_rsa[MAX_CPUS] = {0};
int num_meta_tx[MAX_CPUS] = {0};
int num_meta_rx[MAX_CPUS] = {0};
int num_hws_insert[MAX_CPUS] = {0};
int num_hws_apply[MAX_CPUS] = {0};
int num_app_data[MAX_CPUS] = {0};

uint64_t sum_syn_ch_lat[MAX_CPUS] = {0}; /* syn - ch */
uint64_t max_syn_ch_lat[MAX_CPUS] = {0};
uint64_t sum_ch_sh_sc_shd_lat[MAX_CPUS] = {0}; /* ch - sh/sc/shd */
uint64_t max_ch_sh_sc_shd_lat[MAX_CPUS] = {0};
uint64_t sum_sh_sc_shd_cke_lat[MAX_CPUS] = {0}; /* sh/sc/shd - cke */
uint64_t max_sh_sc_shd_cke_lat[MAX_CPUS] = {0};
uint64_t sum_cke_rsa_decrypt_lat[MAX_CPUS] = {0}; /* cke - rsa decrypt req. */
uint64_t max_cke_rsa_decrypt_lat[MAX_CPUS] = {0};
uint64_t sum_rsa_decrypt_decrypt_rsa_lat[MAX_CPUS] = {0}; /* rsa decrypt latency */
uint64_t max_rsa_decrypt_decrypt_rsa_lat[MAX_CPUS] = {0};
uint64_t sum_decrypt_rsa_meta_tx_lat[MAX_CPUS] = {0}; /* decrypt rsa - meta tx */
uint64_t max_decrypt_rsa_meta_tx_lat[MAX_CPUS] = {0};
uint64_t sum_meta_tx_meta_rx_lat[MAX_CPUS] = {0}; /* meta tx/rx */
uint64_t max_meta_tx_meta_rx_lat[MAX_CPUS] = {0};
uint64_t sum_meta_rx_hws_insert_lat[MAX_CPUS] = {0}; /* meta rx - hws insert */
uint64_t max_meta_rx_hws_insert_lat[MAX_CPUS] = {0};
uint64_t sum_hws_insert_hws_apply_lat[MAX_CPUS] = {0}; /* hws latency */
uint64_t max_hws_insert_hws_apply_lat[MAX_CPUS] = {0};
uint64_t sum_hws_apply_app_data_lat[MAX_CPUS] = {0}; /* app data latency */
uint64_t max_hws_apply_app_data_lat[MAX_CPUS] = {0};
...
insert_tcp_connection(thread_context_t* ctx, uint16_t portid,
                      ...
                      uint16_t window, uint16_t payload_len);

static tcp_connection_t* 
search_tcp_connection(thread_context_t* ctx,
                      uint32_t client_ip, uint16_t client_port,
                      uint32_t server_ip, uint16_t server_port);
...
static int
validate_syn_packet(tcp_connection_t* conn, 
                  uint32_t seq_no, uint32_t ack_no);

static int
validate_sequence(tcp_connection_t* conn, 
                  uint32_t seq_no, uint32_t ack_no, 
...
print_stat()
{
    ...
    /* Per-Core Stat */
    for (i = 0; i < num_core; i++) {
        ctx_array[i]->cur_stat.throughput 
            = ctx_array[i]->cur_stat.completes - ctx_array[i]->prev_stat.completes;
        ...
        fprintf(stderr,
                ...
                ctx_array[i]->cur_stat.throughput, 
                ctx_array[i]->cur_stat.only_tcp_throughput,
                ctx_array[i]->active_cnt, 
                ctx_array[i]->free_cnt,
        ...
        RTE_ETH_FOREACH_DEV(port) {
            fprintf(stderr,
                    ...
                    i, port,
                    ctx_array[i]->cur_stat.rx_pkts[port] - ctx_array[i]->prev_stat.rx_pkts[port],
                    B_TO_Mb((float)(ctx_array[i]->cur_stat.rx_bytes[port] - ctx_array[i]->prev_stat.rx_bytes[port])),
                    ctx_array[i]->cur_stat.tx_pkts[port] - ctx_array[i]->prev_stat.tx_pkts[port],
                    B_TO_Mb((float)(ctx_array[i]->cur_stat.tx_bytes[port] - ctx_array[i]->prev_stat.tx_bytes[port])),
                    ctx_array[i]->cur_stat.rtx_pkts[port] - ctx_array[i]->prev_stat.rtx_pkts[port],
                    B_TO_Mb((float)(ctx_array[i]->cur_stat.rtx_bytes[port] - ctx_array[i]->prev_stat.rtx_bytes[port])));
            ctx_array[i]->prev_stat.rx_pkts[port] = ctx_array[i]->cur_stat.rx_pkts[port];
            ctx_array[i]->prev_stat.rx_bytes[port] = ctx_array[i]->cur_stat.rx_bytes[port];
            ctx_array[i]->prev_stat.tx_pkts[port] = ctx_array[i]->cur_stat.tx_pkts[port];
            ctx_array[i]->prev_stat.tx_bytes[port] = ctx_array[i]->cur_stat.tx_bytes[port];
            ctx_array[i]->prev_stat.rtx_pkts[port] = ctx_array[i]->cur_stat.rtx_pkts[port];
            ctx_array[i]->prev_stat.rtx_bytes[port] = ctx_array[i]->cur_stat.rtx_bytes[port];
        }
    }

    /* Global Stat */
    cur_global_stat.throughput = cur_global_stat.completes - prev_global_stat.completes;
    cur_global_stat.only_tcp_throughput = cur_global_stat.only_tcp - prev_global_stat.only_tcp;
    ...
    RTE_ETH_FOREACH_DEV(port) {
        ...
        fprintf(stderr,
                ...
                B_TO_Mb((float)(cur_global_stat.rx_bytes[port] - prev_global_stat.rx_bytes[port])),
                cur_global_stat.tx_pkts[port] - prev_global_stat.tx_pkts[port],
                B_TO_Mb((float)(cur_global_stat.tx_bytes[port] - prev_global_stat.tx_bytes[port])),
                cur_global_stat.rtx_pkts[port] - prev_global_stat.rtx_pkts[port],
                B_TO_Mb((float)(cur_global_stat.rtx_bytes[port] - prev_global_stat.rtx_bytes[port])));
    ...
    for (int i = 0; i < rte_lcore_count(); i++) {
        if (ctx_array[i]) {
            ...
            fprintf(stderr, "Local active_sessions of core %d: %ld\n", ctx_array[i]->coreid, ctx_array[i]->active_session_table->count);
        }
    }
            ...
            if (ctx_array[i] && ctx_array[i]->active_session_table) {
                ...
                for (int b = 0; b < ht->bins; b++) {
                    ...
                    TAILQ_FOREACH(sess, &ht->ht_table[b], active_session_link) {
                        fprintf(stderr,
                                "Core %d bucket %d -> client_port: %u\n",
                                ctx_array[i]->coreid, b,
                                ntohs(sess->client_port));
                    }
                }
            }
        }
...
/* for debugging */
const uint8_t TARGET_SRC_MAC[6] = {0x10, 0x70, 0xfd, 0x86, 0x5c, 0x8a};
const uint8_t BROADCAST_MAC[6] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};


int 
is_target_src_mac(const void *data)
{
    const uint8_t *buf = (const uint8_t *)data;

    // src MAC starts at offset 6
    return memcmp(&buf[6], TARGET_SRC_MAC, 6) == 0;
}
...
clear_tcp_connection(tcp_connection_t* conn) 
{
    ...
    conn->pending_pkt_hdr.ethh = (struct rte_ether_hdr){0};
    conn->pending_pkt_hdr.iph = (struct rte_ipv4_hdr){0};
    conn->pending_pkt_hdr.tcph = (struct rte_tcp_hdr){0};
    conn->pending_pkt_hdr.payload_len = 0;
...
clear_ssl_session(ssl_session_t* sess) 
{
    ...
    memset(&sess->client_write_MAC_secret, 0, sizeof(sess->client_write_MAC_secret));
    memset(&sess->server_write_MAC_secret, 0, sizeof(sess->server_write_MAC_secret));
...
thread_local_init(int core_id) 
{
    ...
    for (j = 0; j < local_max_conn; j++) {
        ...
        if (sess->pka_op == NULL) {
            fprintf(stderr, "Cannot allocate memory for "
					"%dth pka op of core[%d]\n",
					j, rte_lcore_id());
            exit(EXIT_FAILURE);
        }

        sess->rsa_operand = calloc(1, sizeof(ctx_example));
        ...
        if (sess->rsa_operand->buf_ptr == NULL) {
            fprintf(stderr, "Cannot allocate memory for "
					"buf of %dth rsa operand's buf of core[%d]\n",
					j, rte_lcore_id());
            exit(EXIT_FAILURE);
        }
    ...
    ctx->rsa_result = malloc_results(1, MAX_BYTE_LEN + 8);
    ...
    /* Create rte_ring for submit pka ring */    
    int ring_size = msb_value(local_max_conn);
    
    char submit_pka_ring_name[32];

    snprintf(submit_pka_ring_name, 
        sizeof(submit_pka_ring_name),
    ...
    ctx->submit_pka_ring = sj_ring_create(submit_pka_ring_name, ring_size);
    ...
    snprintf(completed_pka_ring_name, 
        sizeof(completed_pka_ring_name),
    ...
    ctx->completed_pka_ring = sj_ring_create(completed_pka_ring_name, ring_size);

    if (!ctx->completed_pka_ring)
        rte_exit(EXIT_FAILURE, "Failed to create ring for completed pka ring\n");
    
    /* Insert local_max_conn * 5 records into record_pool */
    TAILQ_INIT(&ctx->record_pool);
    TAILQ_INIT(&ctx->whole_record);

    for (i = 0; i < local_max_conn * 5; i++) {
    ...
    snprintf(waiting_hws_del_ring_name, 
        sizeof(waiting_hws_del_ring_name),
    ...
    //                                             SOCKET_ID_ANY, 
    //                                             RING_F_SP_ENQ | RING_F_SC_DEQ);
    ctx->waiting_hws_del_ring = sj_ring_create(waiting_hws_del_ring_name, ring_size);
...
{
#if VERBOSE_TCP_HS || VERBOSE_CONN_LAT
    num_completed_sess[sess->coreid]++;
#endif /* VERBOSE_TCP_HS || VERBOSE_CONN_LAT */
    ...
    uint64_t conn_lat = (conn_end_time.tv_sec - sess->parent->conn_start_time.tv_sec) * 1000000000L
    ...
    uint64_t hws_apply_app_data_lat = (conn_end_time.tv_sec - sess->hws_apply_time.tv_sec) * 1000000000L
                        + (conn_end_time.tv_nsec - sess->hws_apply_time.tv_nsec);
    
    if (hws_apply_app_data_lat > max_hws_apply_app_data_lat[sess->coreid]) {
        max_hws_apply_app_data_lat[sess->coreid] = hws_apply_app_data_lat;
    }
    
    sum_hws_apply_app_data_lat[sess->coreid] += hws_apply_app_data_lat; 
...
{
    /* Send RST packet */
    if (unlikely(send_tcp_packet(sess->parent,
				    			 NULL,
								 0,
                   ...
                   uint32_t cookie)
{
    ...
    sprintf(syn_dst_hw, "%x:%x:%x:%x:%x:%x",
            syn_ethh->dst_addr.addr_bytes[0],
            syn_ethh->dst_addr.addr_bytes[1],
            syn_ethh->dst_addr.addr_bytes[2],
            syn_ethh->dst_addr.addr_bytes[3],
            syn_ethh->dst_addr.addr_bytes[4],
            syn_ethh->dst_addr.addr_bytes[5]);

    sprintf(syn_src_hw, "%x:%x:%x:%x:%x:%x",
            syn_ethh->src_addr.addr_bytes[0],
            syn_ethh->src_addr.addr_bytes[1],
            syn_ethh->src_addr.addr_bytes[2],
            syn_ethh->src_addr.addr_bytes[3],
            syn_ethh->src_addr.addr_bytes[4],
...
send_meta_packet(int core_id, int port, tcp_connection_t* conn)
{
    ...
    int total_len = mac_len + eth_type_len + client_port_len + sizeof(conn_meta_t) + 
                    byte_of_send_buffer_offset + 
    ...
    meta_hdr.server_seq_num = htonl(conn->last_recv_ack + SVR_CCS_FIN_SEG_LEN);
    meta_hdr.server_ack_num = htonl(conn->last_recv_seq + CLI_KEX_CCS_FIN_SEG_LEN);
    meta_hdr.custom_eth_type = htons(key_eth_types[idx]);

    build_connection_meta(conn->ssl_session, &meta_hdr.conn_meta);

    uint8_t* buf = get_wptr(core_id, port, total_len, 0, 
                            sizeof(meta_hdr_t) + 
                            byte_of_send_buffer_offset + 
                            conn->ssl_session->send_buffer_offset);
    if (unlikely(!buf)) {
    ...
    memcpy(buf + sizeof(meta_hdr_t), 
           &conn->ssl_session->send_buffer_offset, 
           byte_of_send_buffer_offset);

    memcpy(buf + sizeof(meta_hdr_t) + byte_of_send_buffer_offset, 
           &conn->ssl_session->send_buffer, 
           conn->ssl_session->send_buffer_offset);
        ...
        fprintf(stderr, "\nTCP/TLS Meta Sending Info---------------------------------\n"
                ...
                (port_info[2] << 8) | port_info[3],
                buf[mac_len], buf[mac_len + 1],
                total_len);
    }
#endif /* VERBOSE_TCP */
				 ...
				 uint8_t* payload, uint16_t payload_len) 
{
    ...
    buf = get_wptr(coreid, port, TOTAL_HEADER_LEN + payload_len, 0, TCP_HEADER_LEN);
    assert(buf != NULL);
...
int
send_tcp_packet(tcp_connection_t* conn,
                uint8_t* payload,
                uint16_t payload_len,
                uint8_t flags,
				uint8_t offl_flag) {
	...
	do {
		...
		buf = get_wptr(conn->coreid, conn->portid, TOTAL_HEADER_LEN + byte_to_send, 1, TCP_HEADER_LEN);
#endif /* !OFFLOAD_AES_GCM */
                   ...
                   uint16_t window, uint16_t payload_len)
{
    ...
    target->cookie = get_cookie(client_ip, client_port,
...
static int
validate_packet_type(uint16_t port, uint8_t* pktbuf, uint16_t payload_len) /* TODO: readability */
{
    struct rte_ether_hdr* ethh;
    struct rte_ipv4_hdr* iph;
    struct rte_tcp_hdr* tcph;
...
validate_syn_packet(tcp_connection_t* conn, 
                  uint32_t seq_no, uint32_t ack_no)
{
    if (likely(seq_no == conn->last_recv_seq &&
               ack_no == conn->last_recv_ack)) {
#if VERBOSE_TCP
        fprintf(stderr, "New SYN Packet Received Again!\n");
#endif /* VERBOSE_TCP */
                  ...
                  uint16_t payload_len)
{
	...
	fprintf(stderr, "Invalid Seqence! \n"
			...
			seq_no,
			conn->last_sent_ack, conn->last_recv_seq + conn->last_recv_len,
			conn->last_sent_seq, conn->last_sent_ack, conn->last_sent_len,
			conn->last_recv_seq, conn->last_recv_ack, conn->last_recv_len);
#endif /* VERBOSE_TCP */
...
static void
process_normal_seq_case(tcp_connection_t* result, 
                        uint32_t seq_no, uint32_t ack_no,
                        uint8_t* payload, uint16_t payload_len)
{
    int ret;
    ...
    /* Got a payload */
    if (unlikely(process_ssl_packet(result,
                                    payload,
                                    payload_len) < 0)) {
        /* We need next packet */
...
static void
process_under_seq_case(tcp_connection_t* conn, 
                       uint32_t seq_no, uint32_t ack_no,
                       uint16_t payload_len)
{
    int ret;
...
// #if !TEST
//     fprintf(stderr, "retransmitted client packets are dropped\n");
//     abort_session(conn->ssl_session);
// #endif /* !TEST */

#if VERBOSE_TCP
    fprintf(stderr, "Retransmission!, seq_no: %u, last: %u, current_ssl_state: %d\n",
            seq_no, conn->last_recv_seq, conn->ssl_session->handshake_state);
#endif /* VERBOSE_TCP */
    ...
    fprintf(stderr, "Retransmission!, client_port = %u, seq_no: %u, last: %u,\n"
            ...
            seq_no, conn->last_recv_seq, ack_no, conn->last_recv_ack,
            conn->ssl_session->handshake_state);
#endif /* DEBUG_FLAG */

    /* Retransmission with payload */
    if (payload_len > 0) {
        int hs = conn->ssl_session->handshake_state;
        ret = 0;
...
#endif /* VERBOSE_TCP */
            ret += send_server_hello(conn->ssl_session);
            ret += send_certificate(conn->ssl_session);
            ret += send_server_hello_done(conn->ssl_session);
            ret += 3 * TOTAL_HEADER_LEN;
        } else if (hs > CLIENT_FINISHED) {
#if VERBOSE_TCP
            fprintf(stderr, "Retransmit server finish\n");
#endif /* VERBOSE_TCP */
            security_params_t* pending_sp = &conn->ssl_session->pending_sp;
            security_params_t* write_sp = &conn->ssl_session->write_sp;

            memset(write_sp, 0, sizeof(*write_sp));
            ret += send_change_cipher_spec(conn->ssl_session);

            memcpy(write_sp, pending_sp, sizeof(*pending_sp));
            conn->ssl_session->send_seq_num_ = 0;
            conn->ssl_session->server_write_IV_seq_num = 0;

            ret += send_server_finish(conn->ssl_session);
...
#if VERBOSE_STAT
        conn->ctx->cur_stat.rtx_pkts[conn->portid] += 1;
        conn->ctx->cur_stat.rtx_bytes[conn->portid] += ret;
#endif /* VERBOSE_STAT */
    }
...
static void
process_handshake_packet(thread_context_t* ctx, uint16_t port,
                         uint8_t* pktbuf, uint8_t* payload, uint16_t payload_len,
                         struct rte_ether_hdr* ethh,
                         struct rte_ipv4_hdr* iph,
                         struct rte_tcp_hdr* tcph,
                         uint32_t seq_no, uint32_t ack_no)
{
    ...
    /* Handle Established Packet */
    result = search_tcp_connection(ctx,
...
#if REMOVE_HWS
    if (result->ssl_session->state == STATE_ACTIVE) {
        /* The connection is already onloaded to host */
        assert(!(result->portid % 2));

        struct rte_tcp_hdr* tcph = (struct rte_tcp_hdr *)(pktbuf + 
        ...
        wbuf = get_wptr(result->coreid, result->portid + 1, len, 1, tcp_hdr_len);
        assert(wbuf != NULL);
#if VERBOSE_TCP
    ...
    switch (seq_case) {
        case TCP_SEQ_OK:
            process_normal_seq_case(result, seq_no, ack_no, payload, payload_len);
            return;
        case TCP_SEQ_UNDER:
...
static void
save_new_syn(tcp_connection_t* conn,
             struct rte_ether_hdr* ethh,
             struct rte_ipv4_hdr* iph,
             struct rte_tcp_hdr* tcph,
             uint16_t payload_len)
{
    conn->pending_pkt_hdr.ethh = *ethh;
    conn->pending_pkt_hdr.iph = *iph;
    conn->pending_pkt_hdr.tcph = *tcph;
    conn->pending_pkt_hdr.payload_len = payload_len;

    return;
}

static void
process_syn_packet(thread_context_t* ctx, uint16_t port,
                   uint8_t* pktbuf, uint16_t payload_len, uint16_t len,
                   struct rte_ether_hdr* ethh,
                   struct rte_ipv4_hdr* iph,
                   struct rte_tcp_hdr* tcph,
                   uint32_t seq_no, uint32_t ack_no)
...
#if !REMOVE_HWS
                if (result->hws_applied && result->hws_ready_to_del) {
...
#if LINUX_TCP_SERVER
    result = insert_tcp_connection(ctx, port,
...
#if VERBOSE_HOST
    fprintf(stderr, "TCP connection is inserted in core %d\n", core_id);
...
static void
process_ack_packet(thread_context_t* ctx, uint16_t port,
                   uint8_t* pktbuf, uint16_t payload_len,
                   struct rte_ether_hdr* ethh,
                   struct rte_ipv4_hdr* iph,
                   struct rte_tcp_hdr* tcph,
                   uint32_t seq_no, uint32_t ack_no)
{
    ...
    /* Forward ACK of application data */
    if (result->ssl_session->state == STATE_ACTIVE) {
        /* The connection is already onloaded to host */
        assert(!(result->portid % 2));
...
#if USE_RTE_HWS
static int
process_host_mig_fin(thread_context_t* ctx,
                     struct rte_ether_hdr* ethh,
                     struct rte_ipv4_hdr* iph,
                     struct rte_tcp_hdr* tcph,
                     int len, int pkt_type)
{
    ...
    target = search_tcp_connection(ctx,
                                iph->src_addr, tcph->src_port,
                                iph->dst_addr, tcph->dst_port);

#if VERBOSE_HOST
    fprintf(stderr, "Try to find connection for mig fin packet from host in core %d\n", ctx->coreid);
    fprintf(stderr, "client_ip: 0x%08X, client_port: 0x%04X\n", iph->src_addr, tcph->src_port);
    fprintf(stderr, "server_ip: 0x%08X, server_port: 0x%04X\n", iph->dst_addr, tcph->dst_port);
#endif /* VERBOSE_HOST */

    if (target) {
        target->recv_mig_fin = 1;
        PRINT_CUR_TIME("set hws available");

#if VERBOSE_CONN_LAT
        clock_gettime(CLOCK_MONOTONIC, &target->ssl_session->meta_rx_time);
        uint64_t meta_tx_meta_rx_lat = (target->ssl_session->meta_rx_time.tv_sec - target->ssl_session->meta_tx_time.tv_sec) * 1000000000L
            + (target->ssl_session->meta_rx_time.tv_nsec - target->ssl_session->meta_tx_time.tv_nsec);
        
        if (meta_tx_meta_rx_lat > max_meta_tx_meta_rx_lat[target->ssl_session->coreid]) {
            max_meta_tx_meta_rx_lat[target->ssl_session->coreid] = meta_tx_meta_rx_lat;
        }
...
static int
process_host_close(thread_context_t* ctx,
                   struct rte_ether_hdr* ethh,
                   struct rte_ipv4_hdr* iph,
                   struct rte_tcp_hdr* tcph,
                   int len, int pkt_type)
...
#else /* LINUX_TCP_SERVER */
static int
process_mtcp_host_close(thread_context_t* ctx,
                   struct rte_ether_hdr* ethh,
                   struct rte_ipv4_hdr* iph,
                   struct rte_tcp_hdr* tcph,
                   int len, int pkt_type)
...
static int
process_host_packet(thread_context_t* ctx, uint16_t port,
                    uint8_t* pktbuf, int len, int pkt_type)
{
    uint8_t* wbuf;
    struct rte_ether_hdr* ethh = (struct rte_ether_hdr *)pktbuf;
    struct rte_ipv4_hdr* iph = (struct rte_ipv4_hdr *)(ethh + 1);
...
static int
process_fin_packet(thread_context_t* ctx, uint16_t port,
                   uint8_t* pktbuf, int len)
{
    ...
    if (target) {
        if (target->onload == TRUE) {
			...
			buf = get_wptr(ctx->coreid, port + 1, len, 1, (tcph->data_off >> 4) << 2);
			assert(buf != NULL);
...
static int
process_rst_packet(thread_context_t* ctx, uint16_t port,
                   uint8_t* pktbuf, int len)
{
    tcp_connection_t* target;
    int ret = FALSE;
	...
	if (target) {
        if (target->onload == TRUE) {
			...
			remove_session(target->ssl_session);
			send_raw_reset(ctx->coreid, port,
						   ethh->src_addr.addr_bytes, ethh->dst_addr.addr_bytes,
...
process_packet(uint16_t core_id, uint16_t port, uint8_t* pktbuf, uint16_t len)
{
    ...
    switch(pkt_type) {
        case SSL_HANDSHAKE:
            process_handshake_packet(ctx, port,
                                     pktbuf, payload, payload_len,
                                     ethh, iph, tcph,
                                     seq_no, ack_no);
            return;
        case TCP_SYN:
            process_syn_packet(ctx, port,
                               pktbuf, payload_len, len,
                               ethh, iph, tcph,
                               seq_no, ack_no);
            return;
        case TCP_ACK:
            process_ack_packet(ctx, port,
                               pktbuf, payload_len,
                               ethh, iph, tcph,
                               seq_no, ack_no);
            return;
...
apply_hws(tcp_connection_t* conn) 
{
    ...
    if (conn->recv_mig_fin && !(conn->hws_inserted)) {
#if VERBOSE_CONN_LAT
        clock_gettime(CLOCK_MONOTONIC, &conn->ssl_session->hws_insert_time);
        uint64_t meta_rx_hws_insert_lat = (conn->ssl_session->hws_insert_time.tv_sec - conn->ssl_session->meta_rx_time.tv_sec) * 1000000000L
            + (conn->ssl_session->hws_insert_time.tv_nsec - conn->ssl_session->meta_rx_time.tv_nsec);
        
        if (meta_rx_hws_insert_lat > max_meta_rx_hws_insert_lat[conn->coreid]) {
            max_meta_rx_hws_insert_lat[conn->coreid] = meta_rx_hws_insert_lat;
        }

        sum_meta_rx_hws_insert_lat[conn->coreid] += meta_rx_hws_insert_lat;
        num_hws_insert[conn->coreid]++;
#endif /* VERBOSE_CONN_LAT */
        ...
        if (unlikely(ret < 0)) {
            fprintf(stderr, 
                "Failed to insert flow rule for port %d\n", conn->portid);
            return;
        }
...
void
handle_after_apply_hws(ssl_session_t* sess)
{
    if (unlikely(sess->handshake_state == SERVER_CIPHER_SPEC &&
        sess->state == STATE_HANDSHAKE &&
        sess->parent->hws_applied)) {
#if VERBOSE_CONN_LAT
        clock_gettime(CLOCK_MONOTONIC, &sess->hws_apply_time);
        uint64_t hws_insert_hws_apply_lat = (sess->hws_apply_time.tv_sec - sess->hws_insert_time.tv_sec) * 1000000000L
            + (sess->hws_apply_time.tv_nsec - sess->hws_insert_time.tv_nsec);
        
        if (hws_insert_hws_apply_lat > max_hws_insert_hws_apply_lat[sess->coreid]) {
            max_hws_insert_hws_apply_lat[sess->coreid] = hws_insert_hws_apply_lat;
        }

        sum_hws_insert_hws_apply_lat[sess->coreid] += hws_insert_hws_apply_lat;
        num_hws_apply[sess->coreid]++;
#endif /* VERBOSE_CONN_LAT */

        send_server_finish(sess);
...
#if !REMOVE_HWS
        MEASURE("delete hws rule",
            ret = del_hws_async(conn);
        );
#else /* REMOVE_HWS */
...
handle_waiting_hws_del_conns(thread_context_t* ctx, uint16_t port)
{
    ...
    for (int i = 0; i < ctx->waiting_hws_del_ring->size; i++) {
        ...
        if (conn->hws_deleted) {
            struct rte_ether_hdr ethh = conn->pending_pkt_hdr.ethh;
            struct rte_ipv4_hdr iph = conn->pending_pkt_hdr.iph;
            struct rte_tcp_hdr tcph = conn->pending_pkt_hdr.tcph;
            uint16_t payload_len = conn->pending_pkt_hdr.payload_len;
...
#if LINUX_TCP_SERVER
            tcp_connection_t* result = insert_tcp_connection(ctx, port,
                                                             ethh.src_addr.addr_bytes,
                                                             iph.src_addr, tcph.src_port,
                                                             ethh.dst_addr.addr_bytes,
                                                             iph.dst_addr, tcph.dst_port,
...
ssloff_main_loop(__attribute__((unused)) void* arg) 
{
    ...
    for (;;) {
        /* Process cryptos by PKA Engine */
        if (ctx->coreid == 0) {
            handle_submitted_pka(ctx);
            handle_completed_pka(ctx);
		...
		RTE_ETH_FOREACH_DEV(port) {
			...
			for (i = 0; i < recv_cnt; i++) {
				...
				if (likely(pktbuf != NULL)) {
                    ...
                    if (!is_broadcast(pktbuf)) {
                        ...
                        PRINT_CUR_TIME("Received Packet");
                        hex_dump(pktbuf, len);
                    }
#endif /* VERBOSE_RECV */
			...
			for (i = 0; i < NUM_BINS; i++) {
				...
				TAILQ_FOREACH(target, &ht->ht_table[i], active_session_link) {
                    ...
                    apply_hws(target);
                    remove_hws(target);
                    handle_after_apply_hws(target->ssl_session);
                }
            }
            ...
            if (unlikely(per_sec_task && ((port == 0) || (port == 1)))) {
                ...
                if (total_imissed && ctx->coreid == 0) {
                    ...
                    fprintf(stderr, 
                        "Total imissed pkts of port %u: %lu,\n",
                        port, total_imissed);
                }
            }
			...
			TAILQ_FOREACH(target, 
                          &ctx->active_session_q, 
                          active_session_link) {
				process_session_read(target->ssl_session);
				process_session_health_check(target);
                apply_hws(target);
        ...
        if (unlikely(per_sec_task)) {
            int tot_num_completed_sess = 0;
            int tot_num_ch = 0;
            int tot_num_sh_sc_shd = 0;
            int tot_num_cke = 0;
            int tot_num_rsa_decrypt = 0;
            int tot_num_decrypt_rsa = 0;
            int tot_num_meta_tx = 0;
            int tot_num_meta_rx = 0;
            int tot_num_hws_insert = 0;
            int tot_num_hws_apply = 0;
            int tot_num_app_data = 0;

            uint64_t tot_sum_conn_lat = 0;
            uint64_t tot_sum_syn_ch_lat = 0;
            uint64_t tot_sum_ch_sh_sc_shd_lat = 0;
            uint64_t tot_sum_sh_sc_shd_cke_lat = 0;
            uint64_t tot_sum_cke_rsa_decrypt_lat = 0;
            uint64_t tot_sum_rsa_decrypt_decrypt_rsa_lat = 0;
            uint64_t tot_sum_decrypt_rsa_meta_tx_lat = 0;
            uint64_t tot_sum_meta_tx_meta_rx_lat = 0;
            uint64_t tot_sum_meta_rx_hws_insert_lat = 0;
            uint64_t tot_sum_hws_insert_hws_apply_lat = 0;
            uint64_t tot_sum_hws_apply_app_data_lat = 0;

            uint64_t max_max_syn_ch_lat = 0;
            uint64_t max_max_ch_sh_sc_shd_lat = 0;
            uint64_t max_max_sh_sc_shd_cke_lat = 0;
            uint64_t max_max_cke_rsa_decrypt_lat = 0;
            uint64_t max_max_rsa_decrypt_decrypt_rsa_lat = 0;
            uint64_t max_max_decrypt_rsa_meta_tx_lat = 0;
            uint64_t max_max_meta_tx_meta_rx_lat = 0;
            uint64_t max_max_meta_rx_hws_insert_lat = 0;
            uint64_t max_max_hws_insert_hws_apply_lat = 0;
            uint64_t max_max_hws_apply_app_data_lat = 0;
            
            for (int i = 0; i < MAX_CPUS; i++) {
                tot_sum_syn_ch_lat += sum_syn_ch_lat[i];
                tot_sum_ch_sh_sc_shd_lat += sum_ch_sh_sc_shd_lat[i];
                tot_sum_sh_sc_shd_cke_lat += sum_sh_sc_shd_cke_lat[i];
                tot_sum_cke_rsa_decrypt_lat += sum_cke_rsa_decrypt_lat[i];
                tot_sum_rsa_decrypt_decrypt_rsa_lat += sum_rsa_decrypt_decrypt_rsa_lat[i];
                tot_sum_decrypt_rsa_meta_tx_lat += sum_decrypt_rsa_meta_tx_lat[i];
                tot_sum_meta_tx_meta_rx_lat += sum_meta_tx_meta_rx_lat[i];
                tot_sum_meta_rx_hws_insert_lat += sum_meta_rx_hws_insert_lat[i];
                tot_sum_hws_insert_hws_apply_lat += sum_hws_insert_hws_apply_lat[i];
                tot_sum_hws_apply_app_data_lat += sum_hws_apply_app_data_lat[i];

                tot_num_ch += num_ch[i];
                tot_num_sh_sc_shd += num_sh_sc_shd[i];
                tot_num_cke += num_cke[i];
                tot_num_rsa_decrypt += num_rsa_decrypt[i];
                tot_num_decrypt_rsa += num_decrypt_rsa[i];
                tot_num_meta_tx += num_meta_tx[i];
                tot_num_meta_rx += num_meta_rx[i];
                tot_num_hws_insert += num_hws_insert[i];
                tot_num_hws_apply += num_hws_apply[i];
                tot_num_app_data += num_app_data[i];
            }

            for (int i = 0; i < MAX_CPUS; i++) {
                max_max_syn_ch_lat = max_max_syn_ch_lat < max_syn_ch_lat[i] ? max_syn_ch_lat[i] : max_max_syn_ch_lat;
                max_max_ch_sh_sc_shd_lat = max_max_ch_sh_sc_shd_lat < max_ch_sh_sc_shd_lat[i] ? max_ch_sh_sc_shd_lat[i] : max_max_ch_sh_sc_shd_lat;
                max_max_sh_sc_shd_cke_lat = max_max_sh_sc_shd_cke_lat < max_sh_sc_shd_cke_lat[i] ? max_sh_sc_shd_cke_lat[i] : max_max_sh_sc_shd_cke_lat;
                max_max_cke_rsa_decrypt_lat = max_max_cke_rsa_decrypt_lat < max_cke_rsa_decrypt_lat[i] ? max_cke_rsa_decrypt_lat[i] : max_max_cke_rsa_decrypt_lat;
                max_max_rsa_decrypt_decrypt_rsa_lat = max_max_rsa_decrypt_decrypt_rsa_lat < max_rsa_decrypt_decrypt_rsa_lat[i] ? max_rsa_decrypt_decrypt_rsa_lat[i] : max_max_rsa_decrypt_decrypt_rsa_lat;
                max_max_decrypt_rsa_meta_tx_lat = max_max_decrypt_rsa_meta_tx_lat < max_decrypt_rsa_meta_tx_lat[i] ? max_decrypt_rsa_meta_tx_lat[i] : max_max_decrypt_rsa_meta_tx_lat;
                max_max_meta_tx_meta_rx_lat = max_max_meta_tx_meta_rx_lat < max_meta_tx_meta_rx_lat[i] ? max_meta_tx_meta_rx_lat[i] : max_max_meta_tx_meta_rx_lat;
                max_max_meta_rx_hws_insert_lat = max_max_meta_rx_hws_insert_lat < max_meta_rx_hws_insert_lat[i] ? max_meta_rx_hws_insert_lat[i] : max_max_meta_rx_hws_insert_lat;
                max_max_hws_insert_hws_apply_lat = max_max_hws_insert_hws_apply_lat < max_hws_insert_hws_apply_lat[i] ? max_hws_insert_hws_apply_lat[i] : max_max_hws_insert_hws_apply_lat;
                max_max_hws_apply_app_data_lat = max_max_hws_apply_app_data_lat < max_hws_apply_app_data_lat[i] ? max_hws_apply_app_data_lat[i] : max_max_hws_apply_app_data_lat;
            }

            fprintf(stderr, "Latency breakdown (ns)\n"
                    "syn to ch (max): %.3f (%lu)\n"
                    "ch to sh_sc_shd (max): %.3f (%lu)\n"
                    "sh_sc_shd to cke (max): %.3f (%lu)\n"
                    "cke to rsa_decrypt (max): %.3f (%lu)\n"
                    "rsa_decrypt to decrypt_rsa (max): %.3f (%lu)\n"
                    "decrypt_rsa to meta_tx (max): %.3f (%lu)\n"
                    "meta_tx to meta_rx (max): %.3f (%lu)\n"
                    "meta rx to hws insert (max): %.3f (%lu)\n"
                    "hws insert to hws apply (max): %.3f (%lu)\n"
                    "hws apply to app data (max): %.3f (%lu)\n",
                    tot_num_ch ? (double)tot_sum_syn_ch_lat / tot_num_ch : 0, max_max_syn_ch_lat,
                    tot_num_sh_sc_shd ? (double)tot_sum_ch_sh_sc_shd_lat / tot_num_sh_sc_shd : 0, max_max_ch_sh_sc_shd_lat,
                    tot_num_cke ? (double)tot_sum_sh_sc_shd_cke_lat / tot_num_cke : 0, max_max_sh_sc_shd_cke_lat,
                    tot_num_rsa_decrypt ? (double)tot_sum_cke_rsa_decrypt_lat / tot_num_rsa_decrypt : 0, max_max_cke_rsa_decrypt_lat,
                    tot_num_decrypt_rsa ? (double)tot_sum_rsa_decrypt_decrypt_rsa_lat / tot_num_decrypt_rsa : 0, max_max_rsa_decrypt_decrypt_rsa_lat,
                    tot_num_meta_tx ? (double)tot_sum_decrypt_rsa_meta_tx_lat / tot_num_meta_tx : 0, max_max_decrypt_rsa_meta_tx_lat,
                    tot_num_meta_rx ? (double)tot_sum_meta_tx_meta_rx_lat / tot_num_meta_rx : 0, max_max_meta_tx_meta_rx_lat,
                    tot_num_hws_insert ? (double)tot_sum_meta_rx_hws_insert_lat / tot_num_hws_insert : 0, max_max_meta_rx_hws_insert_lat,
                    tot_num_hws_apply ? (double)tot_sum_hws_insert_hws_apply_lat / tot_num_hws_apply : 0, max_max_hws_insert_hws_apply_lat,
                    tot_num_app_data ? (double)tot_sum_hws_apply_app_data_lat / tot_num_app_data : 0, max_max_hws_apply_app_data_lat
                    );
        ...
        if (unlikely(per_sec_task)) {
            ...
            double elapsed_sec =
					(cur_ts.tv_nsec - start_ts.tv_nsec) / 1e9 +
					(cur_ts.tv_sec - start_ts.tv_sec);
            
            fprintf(stderr, "pka_get_result min=%lu, max=%lu, avg=%lu\n"
					...
					pka_get_result_cnt,
                    pka_get_result_fail_cnt, pka_get_result_success_cnt);
