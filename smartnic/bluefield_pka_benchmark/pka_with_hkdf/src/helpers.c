...
extern pka_operand_t *rsa_encrypt_key, *rsa_decrypt_key, *rsa_modulus, *rsa_ciphertext;
...
Get_P256_key_pair(pka_handle_t handle, int big_endian)
{
	...
	remote_ec_priv_key = calloc(MAX_OUTSTANDING_CMD_NUM, sizeof(pka_operand_t*));
    remote_ec_pub_key_ep = calloc(MAX_OUTSTANDING_CMD_NUM, sizeof(ecc_point_t*));
	remote_shared_secret_ep = calloc(MAX_OUTSTANDING_CMD_NUM, sizeof(ecc_point_t*));
	
	/* allocate ecc point and curve */
	P256_curve = make_ecc_curve(P256_p_buf, sizeof(P256_p_buf),
    ...
    P256_base_pt = make_ecc_point(P256_curve,
	...
	P256_base_pt_order = make_operand(P256_n_buf,
									 sizeof(P256_n_buf),
									 big_endian);
	...
	for (int i = 0; i < MAX_OUTSTANDING_CMD_NUM; i++) {
	    ec_priv_key[i] = rand_non_zero_integer(handle, 
											   P256_base_pt_order);
	    ec_pub_key_ep[i]  = sync_ecc_multiply(handle, 
											  P256_curve, 
											  P256_base_pt, 
											  ec_priv_key[i]);
		remote_ec_priv_key[i] = rand_non_zero_integer(handle, 
													  P256_base_pt_order);
	    remote_ec_pub_key_ep[i]  = sync_ecc_multiply(handle, 
													 P256_curve, 
													 P256_base_pt, 
													 remote_ec_priv_key[i]);

		if (!ec_priv_key[i]->buf_ptr || 
			!ec_pub_key_ep[i]->x.buf_ptr || 
	...
	/* generate answers */
	for (int i = 0; i < MAX_OUTSTANDING_CMD_NUM; i++) {
		remote_shared_secret_ep[i] = sync_ecc_multiply(handle, P256_curve, ec_pub_key_ep[i], remote_ec_priv_key[i]);
	}
...
{
	/* make room for ec_priv_key and ec_pub_key */
    ec_priv_key_x25519_0 = calloc(MAX_OUTSTANDING_CMD_NUM, sizeof(pka_operand_t*));
    ec_priv_key_x25519_1 = calloc(MAX_OUTSTANDING_CMD_NUM, sizeof(pka_operand_t*));
    ec_pub_key_x25519  = calloc(MAX_OUTSTANDING_CMD_NUM, sizeof(pka_operand_t*));
	remote_ec_priv_key_x25519 = calloc(MAX_OUTSTANDING_CMD_NUM, sizeof(pka_operand_t*));
    remote_ec_pub_key_x25519  = calloc(MAX_OUTSTANDING_CMD_NUM, sizeof(pka_operand_t*));
	remote_shared_secret_po = calloc(MAX_OUTSTANDING_CMD_NUM, sizeof(pka_operand_t*));
    ...
    C255_base_pt = make_mont_ecc_point(X25519_curve,
									   Curve255_bp_u_buf, sizeof(Curve255_bp_u_buf),
									   Curve255_bp_v_buf, sizeof(Curve255_bp_v_buf),
									   big_endian);
	...
	/* generate EC key pair */
	for (int i = 0; i < MAX_OUTSTANDING_CMD_NUM; i++) {
	    ec_priv_key_x25519_0[i] = rand_non_zero_integer(handle, 
											   		  C255_base_pt_order);
	  	ec_priv_key_x25519_1[i] = rand_non_zero_integer(handle, 
											   		  C255_base_pt_order);	
		
	    ec_pub_key_x25519[i]  = sync_mont_ecdh(handle, 
											   X25519_curve, 
											   &C255_base_pt->x, 
											   ec_priv_key_x25519_0[i]);
		remote_ec_priv_key_x25519[i] = rand_non_zero_integer(handle, 
													  C255_base_pt_order);
	    remote_ec_pub_key_x25519[i]  = sync_mont_ecdh(handle, 
													  X25519_curve, 
													  &C255_base_pt->x, 
	...
	/* generate answers */
	for (int i = 0; i < MAX_OUTSTANDING_CMD_NUM; i++) {
		remote_shared_secret_po[i] = sync_mont_ecdh(handle, 
													X25519_curve, 
													ec_pub_key_x25519[i], 
													remote_ec_priv_key_x25519[i]);
	}
...
#if DBG_MODE
		if (answer[i]->r.actual_len == 31) fprintf(stderr, "r's len is 31\n");
		if (answer[i]->s.actual_len == 31) fprintf(stderr, "s's len is 31\n");
#endif /* DBG_MODE */
	}
...
static EVP_KDF_CTX *hkdf_ctx_new(const char *algorithm)
{
...
int hkdf_extract(unsigned char *out, size_t out_len,
...
int hkdf_expand_label(unsigned char *out, size_t out_len,
                      const unsigned char *key, size_t key_len,
                      ...
                      const EVP_MD *md)
{
    ...
    *p++ = OSSL_PARAM_construct_octet_string("key",
                                             (void *)key, key_len);
    *p++ = OSSL_PARAM_construct_octet_string("prefix",
                                             (unsigned char *)label_prefix,
                                             sizeof(label_prefix) - 1);
    *p++ = OSSL_PARAM_construct_octet_string("label",
                                             (unsigned char *)label, label_len);
    *p++ = OSSL_PARAM_construct_octet_string("data",
...
PrintStatistics(uint64_t interval)
{
	...
	if (elapsed_sec > 0) {
		...
		printf("[AVG TOTAL]:  %.2f sess/sec (%.2f Ksess/sec, %.2f Msess/sec)\n",
