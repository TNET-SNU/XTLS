    ...
    for (cmsg = CMSG_FIRSTHDR(&msg); cmsg != NULL; cmsg = CMSG_NXTHDR(&msg, cmsg)) {
        if (cmsg->cmsg_level == SOL_TLS && cmsg->cmsg_type == TLS_GET_RECORD_TYPE) {
            ...
            if (unlikely(record_type == 0x15)) { /* 0x15: TLS Record Type - Alert */
                if (received == 2 && data_buf[0] == 0x01 && data_buf[1] == 0x00) { /* level=1, desc=0: close_notify */
            ...
            if (unlikely(record_type == 0x16 || record_type == 0x14)) { /* 0x16: Handshake, 0x14: Change Cipher Spec */
                return RECV_TLS;
            }
        }
    }
...
void
init_send_state(conn_state_t* conn) 
{
    conn->hdr_sent = 0;
    conn->hdr_offset = 0;
...
int
send_server_records(struct conn_state *conn) 
{
    ssize_t sent = send(conn->sock_fd, 
                        conn->sccs_sf_buf, 
                        conn->record_len, 0);
...
int
send_https_header(int sock, struct conn_state *conn)
{
    if (!(conn->hdr_sent)) {
        if (conn->hdr_len == 0) {
            ...
            int hdr_len = snprintf(conn->hdr_buf, sizeof(conn->hdr_buf),
                ...
                "connection: %s\r\n"
                "\r\n",
                conn->scode, scode_2_str(conn->scode), t_str, conn->file_size,
                  ...
                  struct file_cache *fcache)
{
    ...
    if (conn->scode == 200) {
        ...
        else if (conn->resp_remain > 0) {
            ssize_t fsize = fcache[conn->file_idx].size;
            if (fsize > FILE_SIZE_THRESHOLD) {
            // UNUSED(fsize);
            // if (0) {
                if (conn->resp_fd < 0) {
                    conn->resp_fd = open(fcache[conn->file_idx].fullname, O_RDONLY);
                    if (conn->resp_fd < 0) {
                        perror("open");
                ...
                ssize_t sent = sendfile(sock, conn->resp_fd, &conn->resp_offset, conn->resp_remain);
                ...
                ssize_t sent = send(sock, 
                                    fcache[conn->file_idx].file + conn->resp_offset, 
                                    conn->resp_remain, 0);
                ...
                if (sent > 0) {
                    ...
                    if (errno == ECONNRESET || errno == EPIPE)
                        ...
                        fprintf(stderr, 
                                "NEW errno %d: %s (symbol name: %s)\n", 
                                errno, strerror(errno), strerrorname_np(errno));
...
int
handle_persist_conn_after_resp_sent(int sock, struct conn_state *conn)
{
    // int cork = 0;
...
int
handle_ephe_conn_after_resp_sent(int sock, struct conn_state *conn)
{
    uint8_t alert[2] = {
    ...
    *CMSG_DATA(cmsg) = 0x15;  // TODO: kernel version > 5.19, net/tls_prot.h defines TLS_RECORD_TYPE_ALERT
    msg.msg_controllen = sizeof(ctrl);
...
/*----------------------------------------------------------------------------*/
void
thread_local_init(thread_args_t* thread_args)
{
    ...
    if (unlikely(cur_ts.tv_sec > prev_ts.tv_sec)) {
        ...
        if (total_conns > 0) {
            fprintf(stderr, "Avg connection time: %.2f msec\n",
                ...
                (double)total_conn_time.tv_nsec / total_conns) / 1e6);
        }
...
#if VERBOSE_TCP
                printf("Connected with client %u.%u.%u.%u:%u\n",
                       (((struct conn_state*)events[i].data.ptr)->meta_info.client_ip >> 24) & 0xFF,
                       (((struct conn_state*)events[i].data.ptr)->meta_info.client_ip >> 16) & 0xFF,
                       (((struct conn_state*)events[i].data.ptr)->meta_info.client_ip >> 8) & 0xFF,
                       (((struct conn_state*)events[i].data.ptr)->meta_info.client_ip) & 0xFF,
...
int main(int argc, char **argv) {
    ...
    while (-1 != (o = getopt(argc, argv, "p:v:t:"))) {
        ...
        case 'v':
            if (strcmp(optarg, "tls12") == 0) {
                ...
                fprintf(stderr, 
                    "Only TLS version 1.2 and 1.3 are supported.\n");
                return -1;
            }
            break;
        case 't':
    ...
    for (int i = 0; i < thread_num; i++) {
        ...
        /* create thread */
        int rc = pthread_create(&p_thread[i], &attr[i], worker_thread, &thread_args[i]);
        if (rc != 0) {
