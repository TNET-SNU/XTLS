...
int 
set_mig_sock(struct meta_info* meta_info) 
{
    int sock;
    int yes = 1, val;
    ...
    MEASURE("setsockopt_set_tcp_linger", 
        if (setsockopt(sock, SOL_SOCKET, SO_LINGER, (char*) &Linger, sizeof(Linger)) == -1)
            return pr_perror("setsockopt: linger");
    );

    /* Set socket option - Set seq/ack num */
    val = TCP_SEND_QUEUE;
    MEASURE("setsockopt_set_tcp_send_queue",
        if (setsockopt(sock, SOL_TCP, TCP_REPAIR_QUEUE, &val, sizeof(val)))
    ...
    MEASURE("setsockopt_set_tcp_seq_num",
        if (setsockopt(sock, SOL_TCP, TCP_QUEUE_SEQ, &val, sizeof(val)))
    ...
    MEASURE("setsockopt_set_tcp_ack_num",
        if (setsockopt(sock, SOL_TCP, TCP_QUEUE_SEQ, &val, sizeof(val)))
            return pr_perror("TCP_QUEUE_ACK");
    );
...
mig_tcp_conn(conn_state_t* conn, int epfd) 
{
    ...
    MEASURE("register_epoll_out_mig_sock",
    ...
    MEASURE("bind",
        ...
        if (bind(conn->sock_fd, (struct sockaddr *) &bind_addr, sizeof(bind_addr)))
    ...
    MEASURE("connect",
        ...
        if (connect(conn->sock_fd, (struct sockaddr *) &conn_addr, sizeof(conn_addr))) {
    ...
    MEASURE("setsockopt_set_tcp_repair_options",
        if (setsockopt(conn->sock_fd, SOL_TCP, TCP_REPAIR_OPTIONS,
            opts, onr * sizeof(struct tcp_repair_opt)) < 0) {
            return pr_perror("Can't repair options");
        }

        if (setsockopt(conn->sock_fd, SOL_TCP, TCP_REPAIR, &val, sizeof(val)))
            return pr_perror("TCP_REPAIR");
    );
...
{    
    ...
    MEASURE("setsockopt_set_tcp_ulp", 
        if (setsockopt(conn->sock_fd, SOL_TCP, TCP_ULP, "tls", sizeof("tls")) <0) {
            fprintf(stderr, "Client %s:%d\n", inet_ntoa(addr), conn->meta_info.client_port);
    ...
    MEASURE("setsockopt_set_tcp_cork", 
        setsockopt(conn->sock_fd, IPPROTO_TCP, TCP_CORK, &opt_on, sizeof(opt_on));
    );

    setsockopt(conn->sock_fd, IPPROTO_TCP, TCP_QUICKACK, &opt_on, sizeof(opt_on));

    // Send TLS RX info to kTLS/L5o (KTLS_RX)
    MEASURE("setsockopt_set_tls_rx",
        if (setsockopt(conn->sock_fd, SOL_TLS, TLS_RX, &(conn->crypto_info.crypto_info_rx), 
    ...
    MEASURE("setsockopt_set_tls_tx",
        if (setsockopt(conn->sock_fd, SOL_TLS, TLS_TX, &(conn->crypto_info.crypto_info_tx), 
    ...
    // MEASURE("setsockopt_set_tls_zcsendfile",
    //     if (setsockopt(conn->sock_fd, SOL_TLS, TLS_TX_ZEROCOPY_RO, 
    //                    &zcsendfile, sizeof(zcsendfile)) < 0) {
    //         perror("setsockopt(TLS_TX)");
    //         // exit(EXIT_FAILURE);
    //     }
    // );
...
close_tcp_conn(thread_ctx_t* ctx, struct conn_state *conn) 
{
    ...
    MEASURE("close",
        if (close(conn->sock_fd)) {
            fprintf(stderr, "Failed to close socket %d: %s\n", conn->sock_fd, strerror(errno));
        }
    );

    if (likely(ht_search(ctx->ht_conn, conn->meta_info.client_ip, conn->meta_info.client_port))) {
        ht_remove(ctx->ht_conn, conn);
#if EVALUATION
        closed_conns[ctx->core_id]++;

        clock_gettime(CLOCK_MONOTONIC, &conn->conn_fin);
        total_conn_time.tv_sec += 
            (conn->conn_fin.tv_sec - conn->conn_start.tv_sec);
